...
ETF = getSymbols(src = 'yahoo', 
                 Symbols = c('VUSA.AS', 'EUNL.DE', 'VWCE.DE', 'ISFU.L', 'IBIT'),
                 from = '2017-03-01', to = Sys.Date(), auto.assign = T)
```

# Merging 'closing price' columns together

We are only interested in the closing prices per period so we merge all adjusted closing prices for the funds into on dataframe.
...
newNames <-  c('Vang_S&P_500_Acc', 'iShares_World_Acc', 'Vang_FTSE_World_Acc', 'iShares_FTSE_Dist', 'iShares_Bitcoin')
...
There are missing values in the extracted data. This is because some days are bank holidays
for some markets and so there would be no trade for that day. Also, some funds were not existing at earlier periods and so would have NA for values.
...
ETFSummary <- ETF_Analysis_impute %>% 
  ...
  summarise(MnPVSP500 = min(Vang_S.P_500_Acc),
            ...
            MnPiSBITc = min(iShares_Bitcoin),
            MxPiSBITc = max(iShares_Bitcoin)) %>% 
  tbl_df()
...
```{r Analysing iShares Bitcoin...}
iFT_Analysis_outlier <- ETF_Analysis_impute %>% 
  group_by(Year) %>% 
  summarise(Q1 = quantile(iShares_Bitcoin, probs = 0.25),
            Q3 = quantile(iShares_Bitcoin, probs = 0.75),
            IQR = Q3 - Q1,
            UL = Q3 + 1.5 * IQR,
            LL = Q1 - 1.5 * IQR) %>% 
  tibble()
...
# Plot a box plot
ETF_Analysis_impute %>% 
  ggplot(aes(Year, iShares_Bitcoin)) +
  geom_boxplot() +
  labs(title = 'Box plot for iShares Bitcoin', xlab = 'Year', ylab = 'Price')
```
